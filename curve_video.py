...
def HLS_Sfilter(img, thresh_min=130, thresh_max=255):
    hls = cv2.cvtColor(img, cv2.COLOR_RGB2HLS)
    s_channel = hls[:, :, 1]
...
def perspective_img(img, img0, draw=True):
    ...
    warped_bin = ROU(warped_bin, [mask0])
    warped_bin = ROU(warped_bin, [mask1])
    warped_bin = ROU(warped_bin, [mask2])
    ...
    if draw:
        ...
        area = re_perspective_img(img_ROI)
...
# 先进的滑动窗口函数，使用上述5窗口函数
def adv_slide_window(img, win_num=9, win_wr=0.07, min_p=150):
    # time1 = time.time()
    ...
    # 更新信息，同时判断是否识别到左右车道线
    frame_img.update_info(left_lane_indx, left_lane_indy, right_lane_indx, right_lane_indy)

    # 多项式拟合左右2条曲线，获得多项式值
    left_fit = frame_img.l_mean_fit
    right_fit = frame_img.r_mean_fit
...
def sparsity(img, l_x, r_x, ploty):
    # 二值图，左拟合x值，右拟合x值，y值
    # 用拟合到的曲线x+-2的区域作为范围，共5像素
    # 判断是否识别到车道线
    if frame_img.l_detected:
...
# 显示信息
def show_info(img, img_bin, l_x, r_x, ploty, max_sp=0.6):
    ...
    # 初始化图像
    '''
    img_vanish = np.zeros_like(img)
    # np.polyfit(l_y, l_x, 2)
    # 画出曲线
    # right = np.array(tuple(zip(r_fitx, ploty)), np.int32)
    # left = np.array(tuple(zip(l_fitx, ploty1)), np.int32)
    # cv2.polylines(img_vanish, [right], False, (0, 255, 255), thickness=5)
    cv2.polylines(img_vanish, l1, False, (0, 255, 255), thickness=5)
    # cv2.imshow("add2", img_vanish)
    # cv2.waitKey(0)
    lane_img = re_perspective_img(img_vanish)

    img_add2 = cv2.addWeighted(img, 1, lane_img, 0.7, 0)
    img_add2 = cv2.resize(img_add2, (800, 450))
    '''

    # 显示状态
    '''
    t_state=0
    if radius_param[0] > 1800:
        t_state=frame_img.update_state(0)
    elif radius_param[1] < 0:
        t_state=frame_img.update_state(1)
    else:
        t_state=frame_img.update_state(2)

    if t_state==0:
        cv2.putText(img_add, 'Go Straight! {0}'.format(radius_param[1]), (550, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
        # print(radius_param[0], radius_param[1])
    elif t_state==1:
        cv2.putText(img_add, 'Turn Left! {0}'.format(radius_param[1]), (550, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
        # print(radius_param[0], radius_param[1])
    else:
        cv2.putText(img_add, 'Turn Right! {0}'.format(radius_param[1]), (550, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
        # print(radius_param[0], radius_param[1])

    if radius_param[0] > 1000:
        cv2.putText(img_add, 'Go Straight! {0}'.format(test), (550, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
    elif radius_param[1] < 0:
        cv2.putText(img_add, 'Turn Left! {0}'.format(test), (550, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
    else:
        cv2.putText(img_add, 'Turn Right! {0}'.format(test), (550, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 0), 2)
    '''
    # 计算曲率半径和车辆偏离度
    ...
    # 更新信息
    radio_mean, degree_mean = frame_img.update_laneInfo(radius_param[0], degree)
    # 透视变化调整，此处需要调整*************************************************************
    ...
    # 计算稀疏度
    # 平均曲线
    l_sp_r, r_sp_r = sparsity(img_bin, l_x, r_x, ploty)
    # 当前曲线
    l_fit = frame_img.l_current_fit
    r_fit = frame_img.r_current_fit
    l_fitx = l_fit[0] * ploty ** 2 + l_fit[1] * ploty + l_fit[2]
    r_fitx = r_fit[0] * ploty ** 2 + r_fit[1] * ploty + r_fit[2]
    l_sp_c, r_sp_c = sparsity(img_bin, l_fitx, r_fitx, ploty)
    l_sp = np.min([l_sp_r, l_sp_c])
    r_sp = np.min([r_sp_r, r_sp_c])
    # 更新frame的稀疏度
    l_sp_m, r_sp_m = frame_img.update_sparsity(l_sp, r_sp)
...
# 图像合并
def merge_img(img_add, img_per, img_filter, img_slide):
...
def process_img(img0):
    ...
    l_x, r_x, ploty, img_slide = adv_slide_window(img_warped_bin)
    time4 = time.time()
    img_processed = show_info(img0, img_warped_bin, l_x, r_x, ploty)
    time5 = time.time()
    # cv2.imshow('frame', img_processed)
    result = merge_img(img_processed, img_warped_color, img_warped_bin, img_slide)
    time6 = time.time()
